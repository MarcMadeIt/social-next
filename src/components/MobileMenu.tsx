...
const MobileMenu = () => {
  ...
  return (
    <div className='relative'>
       <ClerkLoaded> 
       <SignedIn>
        <div className='flex gap-[4.5px] cursor-pointer' onClick={()=>setIsOpen(prev=>!prev)}>
        <FaEllipsisVertical className='text-primary' size={20} />
          ...
          {isOpen && (
            <div className=' p-1 absolute -right-4 top-10 border-solid border-2 border-black rounded-xl bg-background flex flex-col items-center justify-center gap-2 font-medium z-10 text-sm text-primary'>
                <Link href="/" className='flex items-center gap-2 hover:text-secondary duration-300 transition-colors w-32 p-2'>
                <Image src="/uk.png" alt='' width={15} height={15} />
                English</Link>
                <Link href="/" className='hover:text-secondary duration-300 transition-colors w-32 p-2 flex gap-2 items-center'><FaBell />Notifications</Link>
                <Link href="/settings" className='flex items-center gap-2 hover:text-secondary duration-300 transition-colors w-32 p-2'><FaCog />Settings</Link>
                 <SignOutButton>
